...
public class Web3jUtility {
    ...
    public static Client initClient(BCOSStubConfig.ChannelService channelServiceConfig)
        ...
        //cryptoType
        int cryptoType = channelServiceConfig.isGmConnectEnable() ? CryptoType.SM_TYPE : CryptoType.ECDSA_TYPE;
    ...
    private static Map<String, Object> buildCryptoMaterial(BCOSStubConfig.ChannelService channelServiceConfig)
            throws WeCrossException, IOException {
        PathMatchingResourcePatternResolver resolver = new PathMatchingResourcePatternResolver();
        Map<String, Object> cryptoMaterial = new HashMap<>();
        if (channelServiceConfig.isGmConnectEnable()) {
            checkCertExistAndPut(resolver, cryptoMaterial, channelServiceConfig.getGmCaCert(), "caCert");
            checkCertExistAndPut(resolver, cryptoMaterial, channelServiceConfig.getGmSslCert(), "sslCert");
            checkCertExistAndPut(resolver, cryptoMaterial, channelServiceConfig.getGmSslKey(), "sslKey");
            checkCertExistAndPut(resolver, cryptoMaterial, channelServiceConfig.getGmEnSslCert(), "enSslCert");
            checkCertExistAndPut(resolver, cryptoMaterial, channelServiceConfig.getGmEnSslKey(), "enSslKey");
        } else {
            checkCertExistAndPut(resolver, cryptoMaterial, channelServiceConfig.getCaCert(), "caCert");
            checkCertExistAndPut(resolver, cryptoMaterial, channelServiceConfig.getSslCert(), "sslCert");
            checkCertExistAndPut(resolver, cryptoMaterial, channelServiceConfig.getSslKey(), "sslKey");
        }
        return cryptoMaterial;
    }

    private static void checkCertExistAndPut(PathMatchingResourcePatternResolver resolver, Map<String, Object> cryptoMaterial, String certLocation, String key)
            throws WeCrossException, IOException {
