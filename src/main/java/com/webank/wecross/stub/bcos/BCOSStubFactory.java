...
public class BCOSStubFactory implements StubFactory {
	...
	public Account newAccount(String name, String path) {
		try {
			return BCOSAccountFactory.build(name, path.startsWith("classpath") ? path : "file:" + path);
		} catch (Exception e) {
	...
	public void generateAccount(String path, String[] args) {
		try {
			...
			KeyPairGenerator keyPairGenerator = KeyPairGenerator.getInstance("EC", Security.getProvider("BC"));
			...
			String accountTemplate = "[account]\n" + 
					"    type='BCOS2.0'\n" + 
